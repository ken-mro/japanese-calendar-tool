              ...
              {JAPANESE_ERAS.map((era) => (
                ...
                >
                  ...
                  <td
                    className="text-xl"
                    style={{ textAlign: "center", padding: "1rem 2rem" }}
                    data-label={isJa ? "読み" : "Romaji"}
                  >
                    {isJa ? era.nameHiragana : era.name}
                  </td>
                  <td
                    className="text-lg font-mono"
                    style={{ textAlign: "center", padding: "1rem 2rem" }}
                    data-label={isJa ? "開始日" : "Start"}
                  >
                  ...
                  <td
                    className="text-lg font-mono"
                    style={{ textAlign: "center", padding: "1rem 2rem" }}
                    data-label={isJa ? "終了日" : "End"}
                  >
                    {era.endYear === 9999
                      ? isJa
                        ? "現在"
                        : "Present"
                      : formatDate(era.endYear, era.endMonth, era.endDay)}
                  </td>
                  <td
                    className="text-lg text-[var(--text-secondary)]"
                    style={{ textAlign: "center", padding: "1rem 2rem" }}
                    data-label={isJa ? "年数" : "Duration"}
                  >
                    {era.endYear === 9999
                      ? "-"
